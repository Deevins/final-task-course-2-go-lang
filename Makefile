...
migrate-ledger: wait-ledger-db
	$(COMPOSE) run --rm ledger sh -c "go install github.com/pressly/goose/v3/cmd/goose@latest && POSTGRES_DSN=$(LEDGER_POSTGRES_DSN) bash /app/ledger/scripts/migrate.sh"
